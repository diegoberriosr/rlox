...
pub fn generate_tokens(source_code: &str) -> Vec<Token> {
    ...
    while !is_at_end(current_index, source_code) {
        ...
        match current_char {
            ...
            '=' => tokens.push(Token {
                token_type: if match_char(&mut current_index, '=', source_code) {
                    TokenType::EqualsEquals
                } else {
                    TokenType::Equals
                },
            }),
            '!' => (), // TODO Not or not equals
            '>' => (), // TODO Greater or greater equals,
            '<' => (), // TODO Less or less equals,
            _ => tokens.push(Token {
...
fn match_char(current_index: &mut usize, expected_char: char, source_code: &str) -> bool {
    if is_at_end(current_index + 1, source_code) {
        return false;
    }

    let next_char: char = source_code.as_bytes()[current_index + 1];
...
mod tests {
    ...
    #[test]
    fn test_generate_tokens_with_not_supported_tokens() {
        let expected_output: Vec<Token> = vec![
            Token {
                token_type: TokenType::Unknown,
            };
    ...
    fn test_match_char() {
        ...
        assert_eq(false, match_char(0, '=', str1));
        assert_eq(false, match_char(0, '=', str2));
        assert_eq(true, match_char(0, '=', str3));
    }
}
